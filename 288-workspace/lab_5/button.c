...
void button_init() {
    ...
    timer_waitMillis(1);        // Small delay to allow clock to stabilize

     // 2) Set the buttons (PE0-PE3) as inputs
     GPIO_PORTE_DIR_R &= ~0x0F;  // Set PE0-PE3 as inputs

     // 3) Enable digital functionality for PE0-PE3
...
uint8_t button_getButton() {
    ...
    if ((GPIO_PORTE_DATA_R & 0x08) == 0) {
        return 4;   // Button 4 (PE3) pressed
    } else if ((GPIO_PORTE_DATA_R & 0x04) == 0) {
        return 3;   // Button 3 (PE2) pressed
    } else if ((GPIO_PORTE_DATA_R & 0x02) == 0) {
        return 2;   // Button 2 (PE1) pressed
    } else if ((GPIO_PORTE_DATA_R & 0x01) == 0) {
        return 1;   // Button 1 (PE0) pressed
    } else {
